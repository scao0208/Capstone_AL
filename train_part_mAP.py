...
class LitAutoTrain(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # uncertainty = - pred_scores * torch.log(pred_scores) - (1 - pred_scores) * torch.log(1 - pred_scores)
        # uncertainty = uncertainty.sum().item()
...
check_best=ModelCheckpoint(
    ...
    filename="3-bdd-fast-{epoch:02d}-{ratio:02d}-{map_50:.2f}"
)
...
def train():
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=4, shuffle=False, collate_fn=collection)
